...
from alert_matcher import active_motion_rules, active_object_class_rules
...
class LogFileAlertChannel(AlertChannel):
    ...
    DEFAULT_LOG_PATH = os.path.join(os.path.expanduser("~"), ".cctv_viewer_alerts.log")

    def __init__(self, path=None):
        self.path = path or self.DEFAULT_LOG_PATH
...
class AlertWorker:
    """Background alert lifecycle tracker for a single camera. Mirrors
    MotionWorker/ObjectDetectionWorker's start/stop shape."""
    ...
    def _run(self):
        while not self._stop_event.is_set():
            try:
                ...
                print(f"[alert_manager] cam={self.cam_id} tick error: {exc}")
...
class AlertManager:
    """Owns one AlertWorker per camera, mirrors MotionManager/
    ObjectDetectionManager's shape. Also owns the list of alert
    ...
    def __init__(self, motion_manager, detection_manager, camera_store, channels=None,
        ...
        self.event_store = event_store
        self.recording_manager = recording_manager
        self._workers = {}  # cam_id -> AlertWorker

    def start_alerts(self, cam_id):
        self.stop_alerts(cam_id)
        worker = AlertWorker(
