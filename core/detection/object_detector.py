...
from core.detection.motion_detector import (
    _denormalize_polygon,  # reuse the same normalized -> pixel helper
)
...
# Resolved through core.paths so it works from any working directory and
...
class ObjectDetectionWorker(BackgroundWorker):
...
class ObjectDetectionManager(WorkerManager):
    """Owns one ObjectDetectionWorker per camera. Also owns the shared, bounded,
    ...
    def get_present_classes(self, cam_id):
        ...
        return set() if worker is None else worker.get_present_classes()
    ...
    def get_active_detections(self, cam_id):
        ...
        worker = self.get(cam_id)
        return [] if worker is None else worker.get_active_detections()
