...
class MotionWorker(BackgroundWorker):
    """Background motion detector for a single camera.

    Thread lifecycle comes from BackgroundWorker; this class only
    implements tick() (one frame's worth of detection) and on_stop()
    (flip the status flag).
    """

    INTERVAL_SECONDS = DETECT_INTERVAL_SECONDS
    LOG_TAG = "motion"
    ...
    def tick(self):
        """One detection pass. Called every INTERVAL_SECONDS by
        BackgroundWorker until stop() is requested."""
...
class MotionManager(WorkerManager):
    ...
    def _make_worker(self, cam_id, **kwargs):
        return MotionWorker(cam_id, self.stream_manager, self.camera_store)

    # ----- back-compat aliases -----------------------------------------
    # Call sites in ui/app.py use these names; new code should call
    # start()/stop() from WorkerManager.
    ...
    def get_result(self, cam_id):
        ...
        return MotionResult() if worker is None else worker.get_result()

    def get_status(self, cam_id):
        worker = self.get(cam_id)
        return MotionStatus.STOPPED if worker is None else worker.get_status()
