...
# as sibling files named by event id -- the same "derive the path
# deterministically from a stable id, no extra DB column" approach
# recording_manager.py uses for segment thumbnails.
#
# Both the DB path and the thumbnail root are resolved lazily through
# core.paths rather than captured at import time, so CCTV_DATA_DIR is
# honoured and so the location no longer depends on where this module
# happens to sit in the package tree.
...
def event_thumbnail_path(event_id):
    ...
    return os.path.join(paths.event_thumbnails_root(), f"event_{event_id}.jpg")
...
class EventStore:
    def __init__(self, path=None):
        self.path = path or paths.events_db()
