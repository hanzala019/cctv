...
File layout
-----------
    <paths.footage_root()>/<camera_id>/<start_time>.mp4
...
# Footage root is resolved lazily through core.paths (which handles the
# PyInstaller frozen case and honours CCTV_DATA_DIR) rather than being
# derived from __file__ -- deriving from __file__ made the location
...
class RecordingWorker(BackgroundWorker):
    ...
    LOG_TAG = "recording_manager"
    # Longer than the default: a tick can be mid-write to disk, and
    ...
    def on_thread_exit(self):
        """Close the open segment on the thread that opened it.

        This used to run in stop() on the caller's thread, after a join
        that can time out -- meaning the VideoWriter could be released
        while this worker was still inside _write_frame. Same
        use-after-free shape StreamWorker had. Closing here also means
        the segment's end_time and file size are written by the thread
        that owns the writer, so a half-written row is not possible.
        """
        self._close_current_segment()

    def on_stop(self):
        """Runs on the caller's thread after the join. Only a
    ...
    def tick(self):
        """Write one frame. BackgroundWorker catches and logs any
        exception, so a bad codec, permission error or full disk logs
        and retries next tick instead of silently killing the thread."""
...
class RecordingManager(WorkerManager):
    ...
    def stop_all(self):
        ...
        is not per-camera, so WorkerManager.stop_all() knows nothing
        about it."""
        super().stop_all()
    ...
    def get_current_segment_id(self, cam_id):
        ...
        return None if worker is None else worker.get_current_segment_id()

    def get_status(self, cam_id):
        worker = self.get(cam_id)
        return RecordingStatus.STOPPED if worker is None else worker.get_status()
